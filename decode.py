...
class decode_marker():
    ...
    def extract_and_decode(self, bgr_image, center):
        ...
        if samples.count(voted_marker_id) > (len(samples)*3/4):
...
# ========== read image ==========
bgr_image = cv2.imread('coded_marker_3.png', cv2.IMREAD_COLOR)
scale_factor = 0.25
bgr_image = cv2.resize(bgr_image, (0, 0), fx=scale_factor, fy=scale_factor)
cx, cy = 543, 317  # Example center coordinates

# ========== decode at known center ==========
decoder.extract_and_decode(bgr_image, (cx,cy))
